...
def installed(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Installed]:
    ...
    try:
        ...
        assert not (REPO_ROOT / ".venv" / "bin" / "shepherd").exists(), (
            "this scenario installed into the repo's own venv"
        )
...
def test_shepherd_controld_starts_binds_and_stops(installed: Installed) -> None:
    ...
    wait plus a `/proc` check, and the exit line the daemon prints is read back,
    ...
    assert not Path(f"/proc/{process.pid}").exists(), "the daemon's pid is still in /proc"
...
def test_the_daemon_wrote_only_into_the_throwaway_home(installed: Installed) -> None:
    ...
    `shepherd-controld` resolves its data directory from `XDG_DATA_HOME`, so an
    environment that failed to scrub would silently open the operator's real
    `~/.local/share/shepherd/shepherd.db`. That is not hypothetical — it is what
    a scratch probe did once while this scenario was being written, which is why
    the check exists rather than the promise.
    """
    for name in ("XDG_DATA_HOME", "XDG_RUNTIME_DIR", "CLAUDE_CONFIG_DIR"):
        assert installed.env[name].startswith(str(installed.home)), (
            name, installed.env[name]
        )
    made = sorted(path.name for path in (installed.home / "xdg_data_home").rglob("*.db"))
    assert made, "the throwaway data directory is empty; the daemon wrote elsewhere"

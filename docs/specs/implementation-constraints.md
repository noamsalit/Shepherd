...
| C13 | signals | Attached sessions give an exit time but never an exit code, and there is no `pid`/`procStart` column, so `crashed` versus `stopped` cannot always be separated. | `hooks/`, `linux-process-git/captures/proc-hooks.jsonl` | M1/M2 |
...
| C22 | docs | Appendix A still uses macOS paths (`/Users/noamsalit/Git/…`) under a Linux target (D39), and its `repo_id` values contradict D48's worktree binding. It needs a rewrite against a real Linux layout. | `orchestrator-platform.md` Appendix A | before M1 |

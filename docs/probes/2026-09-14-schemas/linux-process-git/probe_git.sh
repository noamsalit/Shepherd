...
run git -C "$T/main" worktree add -q -b feat/OXDEV-1 "$T/main-wt/OXDEV-1"
...
echo "\$ cat $T/main-wt/OXDEV-1/.git"; cat "$T/main-wt/OXDEV-1/.git"; echo "[file type: $(stat -c %F "$T/main-wt/OXDEV-1/.git")]"
echo "\$ ls $T/main/.git/worktrees"; ls "$T/main/.git/worktrees"
for f in gitdir commondir HEAD; do echo "\$ cat .git/worktrees/OXDEV-1/$f"; cat "$T/main/.git/worktrees/OXDEV-1/$f"; done
...
mkdir -p "$T/main-wt/OXDEV-1/src"
runin "$T/main-wt/OXDEV-1/src" git rev-parse --show-toplevel --git-common-dir --git-dir
runin "$T/main-wt/OXDEV-1/src" git rev-parse --path-format=absolute --show-toplevel --git-common-dir --git-dir
run git -C "$T/main-wt/OXDEV-1" worktree list --porcelain

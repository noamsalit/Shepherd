#!/usr/bin/env python3
"""Shepherd schema probe, second supplement (2026-09-14): write-policy hazards.

A) prompt-suggestion ghost text in the input box: how it looks in capture-pane (-p and -e),
   and what a bare Enter does while it is shown.
B) programmatic text typed while a permission dialog is up (state needs_you):
   does the text reach the model, or is it consumed by the dialog?

Re-run:  python3 docs/probes/2026-09-14-schemas/tmux-tui/probe_tui_supp2.py
Output:  docs/probes/2026-09-14-schemas/tmux-tui/supp2-<UTC stamp>/
...
RUN = os.path.join(HERE, f"supp2-{STAMP}")
os.makedirs(RUN)
    ...
    try:
        ...
        for k, ptxt in enumerate(prompts):
            ...
            if found:
                ...
                cap("A2-after-bare-enter.txt")
                save("A2-hooks-after-bare-enter.json", json.dumps(seq(H, n1), indent=1) + "\n")
                if any(h["_event"] == "PermissionRequest" for h in hooks(H)[n1:]):
                    tm("send-keys", "-t", T, "Escape")
                wait(H, "Stop", 40, after=n1); time.sleep(2)
                break
        save("A0-suggestion-found.txt", f"suggestion ghost text seen: {found} (after {k + 1} prompts)\n")
        # ---- B: text typed into a permission dialog
        n0 = len(hooks(H))
        time.sleep(3)
        tm("send-keys", "-t", T, "-l", "Use the Bash tool to run exactly this command: touch dialog-probe.txt")
        tm("send-keys", "-t", T, "Enter")
        pr = wait(H, "PermissionRequest", 60, after=n0)
        time.sleep(1)
        cap("B1-permission-dialog.txt")
        tm("send-keys", "-t", T, "-l", "Reply with only the word NOPE")
        time.sleep(1)
        cap("B2-after-typing-into-dialog.txt")
        tm("send-keys", "-t", T, "Enter")
        time.sleep(8)
        cap("B3-after-enter.txt")
        save("B-hooks.json", json.dumps(seq(H, n0), indent=1) + "\n")
        save("B-file-created.txt", f"dialog-probe.txt exists: {os.path.exists(os.path.join(d, 'dialog-probe.txt'))}\n")
        tm("send-keys", "-t", T, "Escape"); time.sleep(2)
    finally:
        tm("kill-server")
